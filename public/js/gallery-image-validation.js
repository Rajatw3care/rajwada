/**
 * Exposed as window.setupGalleryImageFormValidation (not a plain
 * $(function(){...})) because this form is also opened inside the modal on
 * /gallery-images (index.blade.php fetches create/edit and injects it via
 ...
 */
window.setupGalleryImageFormValidation = function() {
    if (!$('#galleryImageForm').length) {
        return;
    }
    ...
    $('#galleryImageForm').validate({
        ...
        messages: {
            ...
            alt_text: { required: 'Alt Text is required', notBlank: 'Alt Text cannot be blank spaces' },
...
$(function() {
    window.setupGalleryImageFormValidation();
});
