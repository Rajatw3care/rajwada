...
<div class="grid grid-cols-1 gap-5">
    ...
    <x-forms.richtext label="Body" name="body" :value="$blogPost->body ?? ''" />

    <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
        <x-forms.input label="Category" name="category" :value="$blogPost->category ?? ''" />
        <x-forms.input label="Tags (comma separated)" name="tags" :value="$blogPost->tags ?? ''" />
    </div>
