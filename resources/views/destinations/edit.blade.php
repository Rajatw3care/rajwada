...
@section('content')
    <x-common.page-breadcrumb pageTitle="Edit Destination" subtitle="Update this featured destination" />

    <div class="form-card-royal">
        <div class="form-card-royal__header">
            <h3 class="font-display text-lg font-semibold text-brand-700 dark:text-gold-200">Edit: {{ $destination->name }}</h3>
        </div>

        <form id="destinationForm" action="{{ route('destinations.update', $destination) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="flex items-center gap-3 border-t border-gold-300/15 pt-5">
                ...
                <a href="{{ route('destinations.index') }}" data-modal-close class="btn-royal-cancel">Cancel</a>
            </div>
        </form>
    </div>
