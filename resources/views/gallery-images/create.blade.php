...
@section('content')
    <x-common.page-breadcrumb pageTitle="Add Gallery Image" subtitle="Add a photo to the public gallery" />

    <div class="form-card-royal">
        <div class="form-card-royal__header">
            <h3 class="font-display text-lg font-semibold text-brand-700 dark:text-gold-200">Add Gallery Image</h3>
        </div>

        <form id="galleryImageForm" action="{{ route('gallery-images.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-5">
            @csrf
            ...
            <div class="flex items-center gap-3 border-t border-gold-300/15 pt-5">
                ...
                <a href="{{ route('gallery-images.index') }}" data-modal-close class="btn-royal-cancel">Cancel</a>
            </div>
        </form>
    </div>
