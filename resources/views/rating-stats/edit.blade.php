...
@section('content')
    <x-common.page-breadcrumb pageTitle="Edit Rating Stat" subtitle="Update this ratings & recognition stat" />

    <div class="form-card-royal">
        <div class="form-card-royal__header">
            <h3 class="font-display text-lg font-semibold text-brand-700 dark:text-gold-200">Edit: {{ $ratingStat->label }}</h3>
        </div>

        <form id="ratingStatForm" action="{{ route('rating-stats.update', $ratingStat) }}" method="POST" class="p-6 space-y-5">
            @csrf
            @method('PUT')
            @include('rating-stats._form')
            ...
            <div class="flex items-center gap-3 border-t border-gold-300/15 pt-5">
                ...
                <a href="{{ route('rating-stats.index') }}" data-modal-close class="btn-royal-cancel">Cancel</a>
            </div>
        </form>
    </div>

    @push('scripts')
        <script src="{{ asset('js/rating-stat-validation.js') }}"></script>
    @endpush
@endsection
