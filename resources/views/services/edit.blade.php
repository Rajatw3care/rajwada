...
@section('content')
    <x-common.page-breadcrumb pageTitle="Edit Service" subtitle="Update this royal experience" />

    <div class="form-card-royal">
        <div class="form-card-royal__header">
            <h3 class="font-display text-lg font-semibold text-brand-700 dark:text-gold-200">Edit: {{ $service->title }}</h3>
        </div>

        <form id="serviceForm" action="{{ route('services.update', $service) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-5">
            @csrf
            @method('PUT')
