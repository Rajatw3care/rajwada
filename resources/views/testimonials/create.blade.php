...
@section('content')
    <x-common.page-breadcrumb pageTitle="Add Testimonial" subtitle="Add a client's words to the homepage slider" />

    <div class="form-card-royal">
        <div class="form-card-royal__header">
            <h3 class="font-display text-lg font-semibold text-brand-700 dark:text-gold-200">Add Testimonial</h3>
        </div>

        <form id="testimonialForm" action="{{ route('testimonials.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-5">
            @csrf
            ...
            <div class="flex items-center gap-3 border-t border-gold-300/15 pt-5">
                ...
                <a href="{{ route('testimonials.index') }}" data-modal-close class="btn-royal-cancel">Cancel</a>
            </div>
        </form>
    </div>

    @push('scripts')
        <script src="{{ asset('js/testimonial-validation.js') }}"></script>
    @endpush
@endsection
