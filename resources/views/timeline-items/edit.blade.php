...
@section('content')
    ...
    <div class="form-card-royal">
        ...
        <form action="{{ route('timeline-items.update', $timelineItem) }}" method="POST" class="p-6 space-y-4">
            @csrf
            @method('PUT')
            ...
            <x-forms.textarea label="Description" name="description" :value="$timelineItem->description" />
            <x-forms.input label="Sort Order" name="sort_order" type="number" :value="$timelineItem->sort_order" />
